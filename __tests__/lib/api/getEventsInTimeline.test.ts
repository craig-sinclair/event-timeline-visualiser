...
describe("Get events in timeline tests", () => {
    ...
    it.each([
        ...
        [[mockEventData[0]], 1],
        [[], 0]
    ])("should fetch and return events of various sizes", async (inputEventData, expectedLength) => {
    ...
    it("should throw error when given empty timeline id", async () => {
        await expect(getEventsInTimeline({timelineID: ""})).rejects.toThrow("Error while fetching events, no timeline ID provided.");
    });
    ...
    it("should throw error when API returns success: false and error message", async () => {
        ...
        await expect(getEventsInTimeline({timelineID: exampleTimelineID})).rejects.toThrow("Database connection failed");
        expect(fetch).toHaveBeenCalledTimes(1);
    });
    ...
    it("should throw error when fetch fails", async () => {
        ...
        await expect(getEventsInTimeline({timelineID: exampleTimelineID})).rejects.toThrow("Network error");
        expect(fetch).toHaveBeenCalledTimes(1);
    });
    ...
    it("should handle fetch timeout", async () => {
        ...
        await expect(getEventsInTimeline({timelineID: exampleTimelineID})).rejects.toThrow("Request timeout");
        expect(fetch).toHaveBeenCalledTimes(1);
    });
