...
const mockExpectedTimelineData: TimelineData = { _id: "1010", title: "test2", events: ["12345"], discussionID: 2020 };
const mockSingleEventData: EventData[] = [{ _id: "12345", overview: "example", dateTime: "2025-01-01 10:00", relevance: 0.5, URLS: [], tags: [] }];

const mockMultipleEventData: EventData[] = [
    { _id: "12345", overview: "example", dateTime: "2025-01-01 10:00", relevance: 0.5, URLS: [], tags: [] },
    { _id: "24680", overview: "example-2", dateTime: "2025-01-02 10:01", relevance: 0.9, URLS: ["example-123"], tags: ["example-tag"] },
];
...
describe("Fetch events test suite", () => {
    ...
    it("Should correctly return data for a single event for valid timeline", async () => {
        mockLean.mockResolvedValue([]).mockResolvedValueOnce(mockExpectedTimelineData).mockResolvedValueOnce(mockSingleEventData);

        const mockRequest = {} as NextRequest;
        const response = await GET(mockRequest, ({params: Promise.resolve({timelineID: "exmaple-id"})}));
        const data = await response.json();
    ...
    it("Should correctly return data for multiple events in valid timeline", async () => {
        mockLean.mockResolvedValue([]).mockResolvedValueOnce(mockExpectedTimelineData).mockResolvedValueOnce(mockMultipleEventData);
